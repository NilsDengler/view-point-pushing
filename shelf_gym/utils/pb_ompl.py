...
class PbOMPLRobot():
    ...
    def __init__(self, id) -> None:
        ...
        joint_idx = [j for j in all_joint_idx if self._is_not_fixed(j)][:6]
...
class PbOMPL():
    def __init__(self, robot, obstacles = [], debug=False) -> None:
        '''
        Args
    ...
    def set_planner(self, planner_name):
        ...
        if planner_name == "PRM":
            ...
            if self.debug: print("{} not recognized, please add it first".format(planner_name))
            return
    ...
    def plan_start_goal(self, start, goal, allowed_time = DEFAULT_PLANNING_TIME):
        ...
        sol_path_list = []
        if solved:
            if self.debug: print("Found solution: interpolating into {} segments".format(INTERPOLATE_NUM))
            # print the path to screen
            sol_path_geometric = self.ss.getSolutionPath()
            if self.debug: print("sparse path: ", sol_path_geometric)
            ...
            if self.debug: print("No solution found")
    ...
    def plan(self, goal, allowed_time = DEFAULT_PLANNING_TIME):
        ...
        res, path = self.plan_start_goal(start, goal, allowed_time=allowed_time)
        return res, path
