...
class Robot():
    ...
    def initialize_gripper(self):
        ...
        self._p.enableJointForceTorqueSensor(
            self.sim.robot_id, self.sim.joints['left_inner_finger_pad_joint'].id)
        self._p.enableJointForceTorqueSensor(
            self.sim.robot_id, self.sim.joints['right_inner_finger_pad_joint'].id)

        # Change the friction of the gripper
        self._p.changeDynamics(
            self.sim.robot_id, self.sim.joints['left_inner_finger_pad_joint'].id, lateralFriction=1)
        self._p.changeDynamics(
            self.sim.robot_id, self.sim.joints['right_inner_finger_pad_joint'].id, lateralFriction=1)
